  ...
  import tEmpty from "@/components/common/empty"; //暂无数据
    export default {
          ...
          choseNorm(id,index){
            ...
            this.$axios.get(api.get,{
              ...
              .then((res)=>{
                ...
                //vuex 引用问题
                localStorage.setItem('CAR_EDIT',JSON.stringify({
                  id:id,
                  detail:res.data,
        ...
        watch:{
          ...
          popupVisible(a){
              a? this.$router.replace({name:'carschose'}):this.$router.replace({name:'cars'})
          }
        }
    }
</script>
...
<style lang="scss" scoped>
  ...
  .t-center{
    ...
    height: 70px;
    margin-top: -70px;
    .text{color: #CCCCCC;margin-bottom: 8px;padding-top: 8px;}
    .logo{width: 37px;}
  }
  //$WtsColor
  .WtsColor{color: $WtsColor}
  .mint-header{
  ...
  .shops{
    background-color: #FFFFFF;margin-top: 9px;
    display: flex;
    padding: 8px 0;
    .shops-checkbox{
      padding-top: 24px;
    ...
    .shop-detail{
      ...
      padding-left: 8px;
      position: relative;
      .name{width: 132px;white-space: nowrap;text-overflow: ellipsis;overflow:hidden;line-height: 20px;font-size: 14px;color: #333333;margin-bottom: 4px;}
      .specs{color: #999999;width: 132px;white-space: nowrap;text-overflow: ellipsis;overflow:hidden;font-size: 14px;margin-bottom: 12px;}
      .bt{
        display: flex;
        justify-content: space-between;
        .price{font-size: 16px;color: #FF5000
  ...
  .settlement{
    ...
    .price{
      color: #666666;
      font-size: 14px;
      padding-left: 70px;line-height: 48px;
      .t{
        font-size: 14px;
        .ice{
          font-size: 18px;
