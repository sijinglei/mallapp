<template>
  <div>
    <t-search @search-list="getSearchList"></t-search>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getSearchList(value){
        var that=this;
        that.$axios.post(api.post,{requrl:api.goods.productlist,searchField:value}).then(res =>{
          var thisList = res.data.list;
          that.productList=thisList;
        })
      },
      getList (cur) {
        var vm=this;
